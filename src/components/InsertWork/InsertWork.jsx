...
const InsertWork = () => {
  ...
  useEffect(() => {
    ...
    colorValue ? setFrontMessage(null) : setFrontMessage('ВНИМАНИЕ! : Значение "color" не определено!');
    workValue ? setFrontMessage(null) : setFrontMessage('ВНИМАНИЕ! : Значение "work" не определено!');
...
//TODO 4) если всё ок --> редирект на страницу Works
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      if (!serverResponse) {
        setBackMessage('Ответ сервера пуст!');
      ...
      console.log(`INSERT request is failed...! ${err.message}`);
      setBackMessage(`INSERT request is failed...! ${err.message}`);
    }
    setIsLoading(false);
  }
  ...
  <section className='changeWork'>
      ...
      <form onSubmit={onSubmit} className='changeWork__form'>
        <label className='changeWork__label'>Set work* :</label>
        <div>
          <InputText
        ...
        <label className='changeWork__label'>Set color :</label>
        <ColorPicker
